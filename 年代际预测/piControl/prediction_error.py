...
koe_idx=np.loadtxt("D:/decadal prediction/results/piControl/CanESM2/koeindex.txt")
...
def find_extrema_indices(data):
    ...
    for i in range(1, len(diff)):
        ...
        if prev_diff > 0 and curr_diff < 0:
            ...
            if month_index in [6, 7, 8]:  # 7、8、9月份对应的索引（0表示1月）
...
#NorCPM1_PDOevents
plt.suptitle('CanESM2', fontsize=25, x=0.5, y=0.965)   # x,y=0.5,0.98 (default)
plt.savefig('D:/decadal prediction/results/piControl/CanESM2/koe_events.png')
plt.show()
#?=====>
...
for i in np.arange(0,lenspb):
    ...
    yrs = spbx[i]
    t1 = int(tquotient[events] * 12-7)   #?   -7、-4、-1、0、2、5、8、11、14、17----- #观测
    t2 = int((tquotient[events] + yrs) * 12-7)                                                                      #预测
...
plt.suptitle('NorCPM1_spb_tos_errors-6', fontsize=20, x=0.5, y=0.98)# x,y=0.5,0.98(默认)
