...
epy_new0=ep0.Fphi.loc[:,[10,20,30,50,100,200,300,400,500,600,700],np.arange(90,0,-6)]
epz_new0=ep0.Fz.loc[:,[10,20,30,50,100,200,300,400,500,600,700],np.arange(90,0,-6)]
div_new0=ep0.Dudt.loc[:,[10,20,30,50,100,200,300,400,500,600,700],np.arange(90,0,-1)]
'''
epy_n=np.empty((488,11,15))
epz_n=np.empty((488,11,15))
div_n=np.empty((488,11,90))

import scipy
from scipy import signal
for j in np.arange(0,11):
    for k in np.arange(0,15):
            epy_n[:,j,k]=scipy.signal.detrend(epy_new0[:,j,k])
            epz_n[:, j, k] = scipy.signal.detrend(epz_new0[:, j, k])
    for i in np.arange(0,90):
            div_n[:, j, i] = scipy.signal.detrend(div_new0[:, j, i])
'''
epy=epy0
epz=epz0
div=div0  #------------------------------------******
...
year_bsh_w=[1998,1999,2003,2004,2011,2015]
...
T10=epz_bshw.mean(dim='time')
T1_0=epz_bsh_w.mean(dim='time')
T1_01=epz_bsh_w1.mean(dim='time')

epz_bhw=T10.data/3+T1_0.data*2/3  #
epz_bhw1=T10.data/2+T1_01.data/2
...
T11=epz_bslw.mean(dim='time')
T1_1=epz_bsl_w.mean(dim='time')
T1_11=epz_bsl_w1.mean(dim='time')

epz_blw=T11.data/3+T1_1.data*2/3  #
epz_blw1=T11.data/2+T1_11.data/2  #
...
T12=epz_oshw.mean(dim='time')
T1_2=epz_osh_w.mean(dim='time')
T1_21=epz_osh_w1.mean(dim='time')

epz_ohw=T12.data/3+T1_2.data*2/3  #
epz_ohw1=T12.data/2+T1_21.data/2  #
...
T13=epz_oslw.mean(dim='time')
T1_3=epz_osl_w.mean(dim='time')
T1_31=epz_osl_w1.mean(dim='time')

epz_olw=T13.data/3+T1_3.data*2/3  #
epz_olw1=T13.data/2+T1_31.data/2  #
...
T20=div_bshw.mean(dim='time')
T2_0=div_bsh_w.mean(dim='time')
T2_01=div_bsh_w1.mean(dim='time')

div_bhw=T20.data/3+T2_0.data*2/3#
div_bhw1=T20.data/2+T2_01.data/2
...
T21=div_bslw.mean(dim='time')
T2_1=div_bsl_w.mean(dim='time')
T2_11=div_bsl_w1.mean(dim='time')

div_blw=T21.data/3+T2_1.data*2/3#
div_blw1=T21.data/2+T2_11.data/2#
...
T22=div_oshw.mean(dim='time')
T2_2=div_osh_w.mean(dim='time')
T2_21=div_osh_w1.mean(dim='time')

div_ohw=T22.data/3+T2_2.data*2/3#
div_ohw1=T22.data/2+T2_21.data/2#
...
T23=div_oslw.mean(dim='time')
T2_3=div_osl_w.mean(dim='time')
T2_31=div_osl_w1.mean(dim='time')

div_olw=T23.data/3+T2_3.data*2/3#
div_olw1=T23.data/2+T2_31.data/2#
...
#冬季----------------------------------------------------------------------
enhance(epy_bhw,epz_bhw)
enhance(epy_blw,epz_blw)
enhance(epy_ohw,epz_ohw)
enhance(epy_olw,epz_olw)
...
div_o_hw1=(T41.data+T42.data)/2   #high(6)  --差值显著性检验
div_o_lw1=(T51.data+T52.data)/2   #low (7)  ——差值..
...
t22,p22=ttest_ind(div_o_ha,div_o_la,equal_var=False)
t33,p33=ttest_ind(div_b_hw1,div_b_lw1,equal_var=False)
t44,p44=ttest_ind(div_o_hw1,div_o_lw1,equal_var=False)

#散度差值
dif11=div_bla-div_bha                                 #巴伦支海秋季
dif22=div_ola-div_oha
dif33=div_blw-div_bhw
dif44=div_olw-div_ohw
...
epy33=epy_blw-epy_bhw
epy44=epy_olw-epy_ohw

epz11=epz_bla-epz_bha
epz22=epz_ola-epz_oha
epz33=epz_blw-epz_bhw
epz44=epz_olw-epz_ohw
...
plt.contourf(lat,level,dif33,levels=np.linspace(-3.0e-5,3.0e-5,11),cmap=cp)#,levels=np.linspace(-2.6e-5,2.6e-5,11)
...
plt.quiver(lat,level,epy33,epz33, width=0.0025,scale=180000000, color='k')
...
ax.set_title('EP-Flux_bw_d-val',fontsize=18)
...
print('hello,world!')
plt.savefig('/home/dell/ZQ19/EP/ep/EP-Flux.jpg')

'''
#创建子图来放置colorbar
ax=fig.add_axes([0.2,0.4,0.6,0.03])
norm =mpl.colors.Normalize(vmin=-3.0e-5, vmax=3.0e-5)
fc1=fig.colorbar(mpl.cm.ScalarMappable(norm=norm,
                 cmap=cp),cax=ax,
                 orientation='horizontal',extend='both')
fc1.set_label('divergence',size=3,rotation=90,labelpad=5,fontsize=18)
'''
